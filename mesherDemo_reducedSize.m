...
% Same algorithm as mesherDemo.m, with reduced #elements for laptop
% compatable calculation.
% Author: Yiren Shen 
% Date: July-19-2023
% Version: 1.0
...
scaleFactor = util.chebyshev(13, [0.0,0.99]);
scaleFactor = scaleFactor(1:ceil(length(scaleFactor)/2));
laminarThickness = 0.03;
...
for ii = 1:length(scaleFactor)
    ...
    z_store = [z_store; unique(z_maped)];
end

num_nodes_planar = size(z_store,1); % calculate number of nodes in a plane 
num_circular_nodes = num_nodes_planar/num_layer_r;
num_planar_nodes = num_nodes_planar;
% group planar O grid into corresponding perimental groups 

perimental_index_group_2d = util.group2DOgrid(num_nodes_circular+2, z_store);
...
lam_layers_index = util.findLamLayersIndex(num_layer_z, num_nodes_circular+2,num_layer_r, all_z_loc, perimental_index_group_2d);
nonZeroColumns = any(lam_layers_index, 1);
lam_layers_index = lam_layers_index(:,nonZeroColumns);

%% Calculate Mesh Connectivity 
%%% Build Hexahedron mesh elements
element_connectivity_mat = util.buildElement(num_nodes_circular+2, num_nodes_planar, num_layer_r, num_layer_z);
element_connectivity_matrix = element_connectivity_mat;
